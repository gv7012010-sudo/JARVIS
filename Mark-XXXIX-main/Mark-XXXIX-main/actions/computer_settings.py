...
def seek_to(minutes: int = 0, seconds: int = 0):
    total_sec = minutes * 60 + seconds
    if total_sec <= 0:
        restart_media()
        return
    time_str = f"{minutes}:{str(seconds).zfill(2)}"
    pyautogui.press("space")
    time.sleep(0.2)
    pyautogui.write(time_str, interval=0.05)
    time.sleep(0.2)
...
ACTION_MAP: dict[str, callable] = {
    ...
    "restart":             restart_media,
    "repeat":              restart_media,
    ...
    "hibernate":           suspend_computer,
}

_DANGEROUS_ACTIONS = {"restart", "shutdown", "suspend", "sleep", "hibernate"}
...
def computer_settings(
    ...
    if action == "restart" or action == "repeat":
        restart_media()
        return "Restarted media from beginning."

    if action == "seek_forward":
        secs = int(value or 10)
        seek_forward(secs)
        return f"Skipped forward {secs}s."

    if action == "seek_backward":
        secs = int(value or 10)
        seek_backward(secs)
        return f"Skipped backward {secs}s."

    if action in ("seek_to", "go_to"):
        if value and ":" in str(value):
            ...
            s = int(value or 0)
        seek_to(m, s)
        return f"Sought to {m}:{str(s).zfill(2)}."
