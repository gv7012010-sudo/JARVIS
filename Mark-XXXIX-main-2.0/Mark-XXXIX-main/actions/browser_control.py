...
class _BrowserSession:
    ...
    async def summarize(self, question: str = "") -> str:
        page = await self._get_page()
        try:
            text = await page.inner_text("body")
        except Exception as e:
        ...
        try:
            ...
            api_key = json.loads(api_path.read_text(encoding="utf-8"))["gemini_api_key"]
            ...
            if question:
                prompt = f"""You are a web analyst. The user asked about the page they're viewing.
Answer clearly and concisely based ONLY on the page content below.
...
Answer:"""
                ...
                prompt = f"""You are a web analyst. Summarize the key information from this page.
Be clear and concise. Use bullet points if helpful. Answer in the same language as the user.
...
Summary:"""
            ...
            return f"Could not analyze page: {e}"
...
def browser_control(
    ...
    try:
        if action == "go_to":
            ...
            sess.run(sess.go_to(params.get("url", "")))
            result = sess.run(sess.summarize(params.get("query", "")))
        elif action == "get_url":
